...
public class RoeselsBushCricket extends Insect {

	public static final int ID = 3;
	public static final String NAME = "Roesel's Bush Cricket";
	public static final int frequency = 14;
	public static final float threshold = 0.5f;
	
	public RoeselsBushCricket() {
		super(ID, NAME);
	}

	@Override
	public float getValue(float[] freqs) {
		float value = (float) (freqs[9] / freqs[4]);
	...
	public LongTermResult getLongTermResult() {
		...
		if ( 	Collections.max(smoothValues) < threshold && 
			...
			if (meanLocal < 1.4) confidence = Math.min(1.1f*confidence, 1.0f);
